...
def main():
    ...
    number = 10
    generator = gen.gen_batches_from_files(files, 1, class_type=None, f_size=None, yield_mc_info=False, swap_col=None)
    for idx in xrange(number):
        print 'plot waveform \t', idx
        wf, _ = generator.next()
        plot_waveforms(wf[0], idx, folderOUT)
    return
...
def plot_waveforms(wf, idx, folderOUT):
    ...
    for i in xrange(wf.shape[0]):
        ...
        axarr[x, y].set_ylim([-20, 760])
        plt.setp(axarr[x, y].get_yticklabels(), visible=False)
        for j in xrange(wf.shape[1]):
            axarr[x, y].plot(time, wf[i, j] + 20. * j, color='k')
