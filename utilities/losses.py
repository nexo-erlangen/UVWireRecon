...
def loss_mean_relative_error(y_true, y_pred):
    ...
    # return K.exp(K.mean(mre, axis=-1))
    return K.mean(mre, axis=-1)

def mean_absolute_error(y_true, y_pred):
    return K.mean(K.abs(y_pred - y_true), axis=-1)

def loss_mean_squared_relative_error(y_true, y_pred):
    """
    Loss function that calculates the mean squared relative error
    :param y_true:
    :param y_pred:
    :return:
    """
    msre = K.square((y_pred - y_true)/y_true)
    return K.mean(msre, axis=-1)

def loss_energy_and_position_reconstruction(y_true, y_pred):
    """
    Loss function that calculates the mean relative error for energy and position calibration
    :param y_true:
    :param y_pred:
    :return: Mean relative energy & position error loss
    """
    loss = mean_absolute_error(norm(y_true[:, 0], 550, 3500), norm(y_pred[:, 0], 550, 3500)) \
        + mean_absolute_error(y_true[:, 1]/320, y_pred[:, 1]/320) \
        + mean_absolute_error(y_true[:, 2]/320, y_pred[:, 2]/320) \
        + mean_absolute_error(fromTimeToZ(y_true[:, 3])/180, fromTimeToZ(y_pred[:, 3])/180)

    # return K.mean(loss, axis=-1)
    return loss

def norm(value, x_min, x_max):
    """
...
def loss_uncertainty_gaussian_likelihood(y_true, y_pred):
    ...
    y_pred_label = y_pred[:, 0]     # K.stop_gradient(y_pred[:, 0])  # prevent that the gradient flows back over the label network
